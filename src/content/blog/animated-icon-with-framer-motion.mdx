...
Lần đầu tiên mình thấy SVG icon động là khi Resend.com ra mắt. Chúng giúp mình cảm giác bớt sự đơn điệu của dashboard khi chỉ toàn màu đen.
Lúc đó, mình đã tự hỏi **bao giờ mình có thể làm được thứ tương tự**.

<video src={video} autoplay loop muted className="w-full h-[400px]" />

Qua hai năm, sau nhiều bài viết trên X, khóa học như [animations.dev](https://animations.dev), hay các bài chia sẻ của [nandafyi](https://x.com/nandafyi), mình đã hiểu thêm về cách SVG hoạt động.
...
Trong bài viết này, thay vì sử dụng _builtin animation_ của SVG hay dùng CSS để animate, chúng ta sẽ sử dụng **Framer Motion** để tạo hiệu ứng cho icon.
...
1. **Đổi qua dùng `motion.path`** import Framer Motion thay vì `path` thông thường để dùng các function animation của Framer.
...
2. [/experiments](https://github.com/ducan-ne/blog/blob/main/src/experiments/2.svg-animations.tsx) code
