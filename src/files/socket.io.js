...
const s = {type: "error", data: "parser error"},
  ...
  o = ({
         type: e,
         data: s
       }, o, h) => n && s instanceof Blob ? o ? h(s) : a(s, h) : i && (s instanceof ArrayBuffer || r(s)) ? o ? h(s) : a(new Blob([s]), h) : h(t[e] + (s || "")),
  a = (t, e) => {
...
const l = "function" == typeof ArrayBuffer, d = (t, n) => {
    if ("string" != typeof t) return {type: "message", data: y(t, n)};
    const i = t.charAt(0);
    if ("b" === i) return {type: "message", data: f(t.substring(1), n)};
    return e[i] ? t.length > 1 ? {type: e[i], data: t.substring(1)} : {type: e[i]} : s
  }, f = (t, e) => {
    if (l) {
      const s = (t => {
  ...
  return new TransformStream({
    transform(t, e) {
      !function(t, e) {
        ...
        const n = s.length;
        let i;
        if (n < 126) i = new Uint8Array(1), new DataView(i.buffer).setUint8(0, n); else if (n < 65536) {
          i = new Uint8Array(3);
...
const A = _.setTimeout, O = _.clearTimeout;
...
const B = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_".split(""), S = 64, N = {};
let x, L = 0, q = 0;
...
class V extends k {
  ...
  create() {
    ...
    try {
      ...
      null === (t = this.opts.cookieJar) || void 0 === t || t.addCookies(s), (s.withCredentials = this.opts.withCredentials), this.opts.requestTimeout && (s.timeout = this.opts.requestTimeout), s.onreadystatechange = () => {
        var t;
...
if (V.requestsCount = 0, V.requests = {}, "undefined" != typeof document) if ("function" == typeof attachEvent) attachEvent("onunload", H); else if ("function" == typeof addEventListener) {
  addEventListener(true ? "pagehide" : "unload", H, !1)
}
...
const z = {
    websocket: class extends C {
                ...
                transform(a, h) {
                  for (n.push(a); ;) {
                    if (0 === i) {
                      ...
                      if (w(n) < 8) break;
                      const t = v(n, 8), e = new DataView(t.buffer, t.byteOffset, t.length), o = e.getUint32(0);
      ...
      request(t = {}) {
        return Object.assign(t, {xd: this.xd, cookieJar: this.cookieJar}, this.opts), new V(this.uri(), t)
      }
...
class X extends k {
  ...
  open() {
    let t;
    if (this.opts.rememberUpgrade && X.priorWebsocketSuccess && -1 !== this.transports.indexOf("websocket")) t = "websocket"; else {
...
function rt(t) {
  ...
  return n.data = ot(s, e), n.attachments = e.length, {packet: n, buffers: e}
}
...
function ht(t, e) {
  ...
  if (Array.isArray(t)) for (let s = 0; s < t.length; s++) t[s] = ht(t[s], e); else if ("object" == typeof t) for (const s in t) Object.prototype.hasOwnProperty.call(t, s) && (t[s] = ht(t[s], e));
  return t
}

const ct = ["connect", "connect_error", "disconnect", "disconnecting", "newListener", "removeListener"], ut = 5;
var pt;
!function(t) {
...
class bt extends k {
  ...
  _sendConnectPacket(t) {
    this.packet({type: pt.CONNECT, data: this._pid ? Object.assign({pid: this._pid, offset: this._lastOffset}, t) : t})
  }
...
function _t(t, e) {
  ...
  const s = function(t, e = "", s) {
    let n = t;
    s = s || "undefined" != typeof location && location, null == t && (t = s.protocol + "//" + s.host), "string" == typeof t && ("/" === t.charAt(0) && (t = "/" === t.charAt(1) ? s.protocol + t : s.host + t), /^(https?|wss?):\/\//.test(t) || (t = void 0 !== s ? s.protocol + "//" + t : "https://" + t), n = Q(t)), n.port || (/^(http|ws)$/.test(n.protocol) ? n.port = "80" : /^(http|ws)s$/.test(n.protocol) && (n.port = "443")), n.path = n.path || "/";
  ...
  let a;
  return e.forceNew || e["force new connection"] || !1 === e.multiplex || o ? a = new vt(n, e) : (kt[i] || (kt[i] = new vt(n, e)), a = kt[i]), s.query && !e.query && (e.query = s.queryKey), a.socket(s.path, e)
}
